...
const Profile = () => {
  ...
  const mainSkills = data.mainSkills.map((skill: string, index: number) => (
    <span
      key={index}
      className={`${index !== 0 ? "border-l border-line" : ""} px-2 text-sm text-pri-font`}
    >
      {skill}
    </span>
  ));
  ...
  return (
    <div className="flex-none flex flex-col items-center lg:w-1/3 xl:w-1/4 min-w-80 pt-20 px-8 md:px-4">
      ...
      />
      <h2 className="text-xxl tracking-wide font-bolder uppercase pt-8 pb-1 text-center">
        {data.name}
      </h2>
